...
describe('normalizeTechnologies', () => {
  ...
  test('accepte un tableau de strings', () => {
    expect(normalizeTechnologies(['React', 'Node.js', 'MongoDB']))
      .toEqual(['React', 'Node.js', 'MongoDB']);
  });

  test('trim les espaces dans un tableau', () => {
    expect(normalizeTechnologies(['  React  ', ' Node.js ']))
      .toEqual(['React', 'Node.js']);
  });

  test('filtre les entrées vides dans un tableau', () => {
    expect(normalizeTechnologies(['React', '', '  ', 'Docker']))
  ...
  test('accepte un seul élément sans virgule', () => {
    expect(normalizeTechnologies('Docker'))
      .toEqual(['Docker']);
  });

  test('convertit les éléments non-string d\'un tableau en string', () => {
    expect(normalizeTechnologies([42, true]))
      .toEqual(['42', 'true']);
  });
});
